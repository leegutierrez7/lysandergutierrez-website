...
export default function PerformanceMonitor() {
    ...
    return (
        <div className="fixed bottom-4 right-4 bg-black/90 text-white text-xs p-3 rounded-lg shadow-lg z-50 font-mono">
            <div className="font-semibold mb-2">Performance Metrics</div>
            <div className="space-y-1">
                {metrics.fcp && (
                    <div className={`flex justify-between gap-4 ${getScoreColor('fcp', metrics.fcp)}`}>
                        <span>FCP:</span>
                        <span>{Math.round(metrics.fcp)}ms</span>
                    </div>
                )}
                {metrics.lcp && (
                    <div className={`flex justify-between gap-4 ${getScoreColor('lcp', metrics.lcp)}`}>
                        <span>LCP:</span>
                        <span>{Math.round(metrics.lcp)}ms</span>
                    </div>
                )}
                {metrics.fid && (
                    <div className={`flex justify-between gap-4 ${getScoreColor('fid', metrics.fid)}`}>
                        <span>FID:</span>
                        <span>{Math.round(metrics.fid)}ms</span>
                    </div>
                )}
                {metrics.cls !== undefined && (
                    <div className={`flex justify-between gap-4 ${getScoreColor('cls', metrics.cls)}`}>
                        <span>CLS:</span>
                        <span>{metrics.cls.toFixed(3)}</span>
                    </div>
                )}
                {metrics.ttfb && (
                    <div className={`flex justify-between gap-4 ${getScoreColor('ttfb', metrics.ttfb)}`}>
                        <span>TTFB:</span>
